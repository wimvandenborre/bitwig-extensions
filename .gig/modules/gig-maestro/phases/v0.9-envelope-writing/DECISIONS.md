...
## 2026-02-28 — Spike: Envelope writing validation results (revised)

**Decision:** Position-jump at stopped transport does NOT create automation data. Position-jump during active playback DOES create automation data. The validated approach for `writeEnvelope` is: (1) start playback, (2) for each point: jump to beat position → touch → setImmediately → untouch, (3) stop playback, (4) restore original position. Minimum sleep of ~100ms between position jump and touch/set is sufficient. `deleteAllAutomation()` does NOT affect arranger automation write state.
**Rationale:** Tested 4 approaches: (a) position-jump at stopped transport — `hasAutomation` stays false. (b) Play/stop per point (brief 100ms play windows) — `hasAutomation` stays false. (c) Continuous playback with timed real-time value changes — `hasAutomation` true. (d) Continuous playback with position jumps between points — `hasAutomation` true. Approach (d) gives deterministic beat placement while requiring only a single play/stop cycle. The Bitwig engine requires the transport to be actively playing for touch automation recording to engage — the audio thread must be running.
**Alternatives considered:** (a) Pure position-jump (no playback) — doesn't work. (b) Real-time playback with sleeps proportional to beat distance — imprecise, slow for widely spaced points. (c) Continuous playback with position jumps — works, fast, deterministic.
**Status:** ACTIVE
**ID:** D-9.2a
